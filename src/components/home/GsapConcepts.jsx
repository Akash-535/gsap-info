...
const GsapConcepts = () => {
  ...
  useEffect(() => {
    ...
    const reduseWidth = window.innerWidth / 1.5;
    gsap.to(cards, {
      x: () => `-${totalScroll - reduseWidth}px`,
      ease: "none",
      scrollTrigger: {
        trigger: section,
        start: "top top",
        ...
        <div ref={cardsRef} className="mt-10 flex space-x-4 w-max px-10">
          {gsapConcepts.map((obj, index) => (
            <Link
              ...
              key={index}
              className="w-[300px]">
              <div className="border border-white rounded-2xl p-4 h-full hover:bg-white/10 transition-all duration-300 cursor-pointer text-center">
                <Heading as="h3" variant="code" className="text-center">
                  {obj.title}
                </Heading>
                <Text className="mt-3 text-center px-1">{obj.description}</Text>
              </div>
            </Link>
          ))}
        </div>
